...
class LSTM:
    def __init__(self, n_classes, hidden_dim=128,  bptt_truncate=-1):
        ...
        V = np.random.uniform(-np.sqrt(1./hidden_dim), np.sqrt(1./hidden_dim), (1, n_classes, hidden_dim))
    ...
    def __theano_build__(self):
       #  E = self.E
        ...
        x = T.fmatrix('x') # matrix, each row is vector of a word
        y = T.lvector('y') #

        def forward_prop_step(x_e, h_t_prev, c_t_prev):
            """

            :param x_e: word embedding vector
            :param h_t_prev: prev hidden layer
            :param c_t_prev: prev c
            :return:
            """

            # Word embedding layer
            # x_e = E[:, x_t]
            print ('x_e ndim', x_e.ndim)
